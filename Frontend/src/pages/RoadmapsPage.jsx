...
import roadmapsData from '../data/roadmaps-data.json'; // Import the JSON data
...
const RoadmapsPage = () => {
  ...
  const getDifficultyColor = (difficulty) => {
    switch (difficulty) {
      case 'Beginner': return 'text-emerald-400 bg-emerald-900/30 border-emerald-400/30';
      case 'Intermediate': return 'text-amber-400 bg-amber-900/30 border-amber-400/30';
      case 'Advanced': return 'text-red-400 bg-red-900/30 border-red-400/30';
              ...
              <div className="flex-1">
                <div className="flex items-center mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-xl text-gray-400 mt-2">{selectedRoadmap.description}</p>
                  </div>
                </div>
                
                <div className="flex flex-wrap gap-4 mb-6">
                  <div className={`px-4 py-2 rounded-full border ${getDifficultyColor(selectedRoadmap.difficulty)}`}>
                    <Target className="w-4 h-4 inline mr-2" />
                    {selectedRoadmap.difficulty}
                  </div>
                  <div className="px-4 py-2 bg-blue-900/30 text-blue-400 rounded-full border border-blue-400/30">
                    <Clock className="w-4 h-4 inline mr-2" />
                    {selectedRoadmap.duration}
                  </div>
                  <div className="px-4 py-2 bg-purple-900/30 text-purple-400 rounded-full border border-purple-400/30">
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              ...
              <div className="bg-gray-900/50 rounded-xl p-6 border border-gray-800">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedRoadmap.jobTitles?.map((title, index) => (
                    <span key={index} className="px-3 py-1 bg-blue-900/30 text-blue-300 rounded-full text-xs border border-blue-400/20">
                      {title}
                    </span>
                  ))}
                </div>
            ...
            <div className="lg:col-span-3">
                  ...
                  <div key={section.id} className="relative">
                    ...
                    <div className="flex items-center mb-6">
                      ...
                      <div className="flex-1">
                        ...
                        <span className="text-xs text-blue-400 bg-blue-900/30 px-2 py-1 rounded-full border border-blue-400/20 mt-2 inline-block">
                          {section.duration}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="ml-4 border-l-2 border-gray-700 pl-8 space-y-6 relative">
                      {section.steps?.map((step, stepIndex) => (
                        <div key={step.id} className="relative bg-gray-900/30 rounded-xl p-6 border border-gray-800 hover:border-gray-700 transition-all">
                          {/* Step Number Circle */}
              ...
              {/* Completion Message */}
              <div className="mt-12 text-center bg-gradient-to-r from-green-900/30 to-emerald-900/30 rounded-xl p-8 border border-green-400/20">
      ...
      <div className="bg-gradient-to-b from-gray-900 to-black">
        <div className="max-w-7xl mx-auto px-6 py-16 text-center">
          <div className="flex items-center justify-center mb-6">
            ...
            <h1 className="text-5xl font-bold bg-gradient-to-r from-white via-blue-100 to-blue-400 bg-clip-text text-transparent">
              Career Roadmaps
            </h1>
          </div>
        ...
        {filteredRoadmaps.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredRoadmaps.map((roadmap) => (
              <div
                key={roadmap.id}
                className="group bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl overflow-hidden border border-gray-700 hover:border-gray-600 transition-all duration-300 cursor-pointer hover:scale-105"
                onClick={() => setSelectedRoadmap(roadmap)}
              >
                {/* Card Header */}
                <div className={`bg-gradient-to-r ${roadmap.color} p-6 relative overflow-hidden`}>
                  <div className="absolute inset-0 bg-black/20"></div>
                  <div className="relative z-10">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-4xl">{roadmap.icon}</span>
                      <span className={`px-3 py-1 rounded-full text-xs font-medium border backdrop-blur-sm ${getDifficultyColor(roadmap.difficulty)}`}>
                        {roadmap.difficulty}
                      </span>
                    </div>
                    <h3 className="text-xl font-bold text-white mb-2">{roadmap.title}</h3>
                    <p className="text-white/90 text-sm">{roadmap.description}</p>
                  </div>
                </div>
                ...
                <div className="p-6">
                  ...
                  <div className="mb-6">
                    ...
                    <div className="text-green-400 font-semibold text-sm">{roadmap.salary?.entry}</div>
                    ...
                    <div className="space-y-2">
                      {roadmap.sections?.slice(0, 3).map((section, index) => (
                        <div key={section.id} className="flex items-center text-sm">
                          <div className="w-5 h-5 rounded border-2 border-blue-500/30 flex items-center justify-center mr-3">
                  ...
                  {/* Action Button */}
                  <button className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white py-3 px-4 rounded-lg font-medium transition-all flex items-center justify-center group">
